...
const HlsStreamRecorder = class {
    ...
    start() {
        ...
        this.uploader = new HlsUploader(this.dst, { collect: this.collect, endpoint: this.s3endpoint });
                ...
                if (this.subreader) {
                    ...
                    this.index.variants.forEach((variant, index) => {
                        ...
                        if (!rec || !rec.localUrl) {
                            ...
                            rec = new Recorder(this.subreader(variantUrl), new URL(dir, this.dst).href, { startOffset: this.startOffset, collect: this.collect, decrypt: this.decrypt, s3endpoint: this.s3endpoint });
                    ...
                    allGroups.forEach((groupItem, index) => {
                        ...
                        if (srcUri) {
                            ...
                            if (!rec || !rec.localUrl) {
                                ...
                                rec = new Recorder(this.subreader(itemUrl), new URL(dir, this.dst).href, { startOffset: this.startOffset, collect: this.collect, decrypt: this.decrypt, s3endpoint: this.s3endpoint });
                                rec.localUrl = Url.format({ pathname: Path.join(dir, 'index.m3u8') });
    ...
    async processInfo(segmentInfo) {
        ...
        let bytesWritten = 0;
        try {
            bytesWritten = await this.writeStream(segmentInfo.stream, uri, meta).bytesWritten;
        }
        catch (err) {
    ...
    async processSegment(segmentInfo) {
        ...
        let bytesWritten = 0;
        try {
            bytesWritten = await this.writeStream(stream, segment.uri, meta).bytesWritten;
        }
        catch (err) {
            Bounce.rethrow(err, 'system');
