...
const HlsStreamRecorder = class HlsStreamRecorder {
    ...
    setupMaster(master, meta) {
        ...
        if (this.subreader) {
            ...
            master.variants = master.variants.filter((variant) => {
                ...
                if (variant.info) {
                    const bw = variant.info.get('bandwidth', M3U8Parse.AttrList.Types.Int);
    ...
    setupMedia(media) {
        ...
        if (!isNaN(this.startOffset)) {
            ...
            media.start = new M3U8Parse.AttrList();
            media.start.set('time-offset', offset, M3U8Parse.AttrList.Types.SignedFloat);
        }
        ...
        if (!this.index) {
            ...
            this.reader.on('close', async () => {
                ...
                try {
                    ...
                    await Promise.all([this.ended.promise, ...this.recorders.map((r) => r.ended.promise)]);
                }
                catch (err) {
                    this.ended.reject(err);
                }
    ...
    async processSegment(segmentInfo) {
        ...
        const segment = new M3U8Parse.MediaSegment(segmentInfo.segment.entry);
