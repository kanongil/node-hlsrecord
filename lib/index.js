...
const HlsStreamRecorder = class {
    ...
    async completed() {
        ...
        try {
            ...
            this.recorders.forEach((r) => !this.reader.stopDate && r.reader.abort());

            await Promise.all(this.recorders.map(async (r) => {

                try {
                    await r.completed();
                }
                catch (err) {
                    console.error(`[${r.localUrl.slice(0, -11)}] recorder crash`, err);
                }
            }));
        }
        finally {
            ...
            if (!this.index.master) {
                ...
                if (this.subreader) {
                    ...
                    this.recorders.forEach((recording) => {
                        ...
                        recording.ended.promise.catch(this.ended.reject);   // Inherit any errors
                        recording.start();
                    });
