...
const HlsStreamRecorder = class HlsStreamRecorder {
        ...
        try {
            ...
            await Promise.race([
                ...
                (async () => {
                    ...
                    if (this.streamer) {
                        await Events.once(this.streamer, 'end');
                    }
                    ...
                    await this.flushIndex({ cacheDuration: this.needsFinalize() ? 60 * 1000 : undefined });
                    await Promise.all(this.recorders.map((r) => r.ended.promise));

                    if (await this.finalizeIndex()) {
    ...
    start(test = true) {
        ...
        assert(!this.index, 'Reader already active');

        const startUploader = () => {

            this.uploader.prepare(this.exclusive, test).catch((err) => {
        ...
        test ? this.reader.once('index', startUploader) : startUploader();
    ...
    setupMaster(master, meta) {
        ...
        master = new M3U8Parse.MasterPlaylist(master);
        ...
        if (this.subreader) {
            ...
            const createRecorder = (dir, url) => {

                const Recorder = /** @type {typeof HlsStreamRecorder} */(this.constructor); // use subclassed HlsStreamRecorder class

                const rec = new Recorder(this.subreader(url), urlWithSearch(dir, this.dst).href, { startOffset: this.startOffset, collect: this.collect, decrypt: this.decrypt });
            ...
            this.uploader.prepared.then(() => {
                ...
                master.variants.forEach((variant, index) => {
                    ...
                    if (!rec || !rec.localUrl) {
                        ...
                        rec = createRecorder(dir, variantUrl);
                ...
                allGroups.forEach((groupItem, index) => {
                    ...
                    if (srcUri) {
                        ...
                        if (!rec || !rec.localUrl) {
                            ...
                            rec = createRecorder(dir, itemUrl);
                            this.recorders.push(rec);
                        }
    ...
    needsFinalize() {
        ...
        return this.index.master; // All master indexes needs to be finalized
    }
    ...
    async processMap(segmentInfo) {
        ...
        const buffer = await streamToBuffer(segmentInfo.stream);
        const hash = Crypto.createHash('sha256').update(buffer);
        const uri = `${this.segmentName(B64.base64urlEncode(hash.digest().slice(0, 8)), true)}.${Mime.extension(meta.mime)}`;

        const { bytesWritten } = await this.writeStream(buffer, uri, meta);
    ...
    async processSegment(segmentInfo) {
        ...
        let bytesWritten = -1;
        try {
            const res = await this.writeStream(stream, segment.uri, meta);
    ...
    segmentName(id, isInit) {

        if (this.collect) {
            return 'stream';
        }

        return (isInit ? 'init-' : '') + id;
    }

    flushIndex(options) {

        return this.uploader.flushIndex(this.index, options);
    }
