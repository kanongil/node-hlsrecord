...
const AwsCreds = require('@aws-sdk/credential-provider-node');
...
class S3UploaderImpl {
    ...
    static async credentials() {
        ...
        if (!S3UploaderImpl._credentials) {
            S3UploaderImpl._credentials = await AwsCreds.defaultProvider()();
        }
    ...
    async pushSegment(stream, name, meta) {
        ...
        const headers = {
            'x-amz-acl': this.ACL,
            'Content-Type': meta.mime || 'video/MP2T',
            'Cache-Control': `max-age=300, s-max-age=${Math.floor(this.cacheDuration / 1000)}, public`
        };
